    ...
    $definitions = [
      ...
      'create_webhook' => [
        ...
        'input_schema' => [
          ...
          'properties' => [
            'name'               => ['type' => 'string'],
            'endpoint_url'       => ['type' => 'string', 'description' => 'May carry {{ dot.path }} placeholders (double braces) expanded at dispatch from the payload the pre-dispatch snippet returned, then the original captured payload — e.g. https://api.github.com/repos/{{ __repo }}/issues. Single braces are sent literally.'],
            'http_method'        => ['type' => 'string', 'enum' => ['GET', 'POST', 'PUT', 'PATCH', 'DELETE'], 'default' => 'POST'],
            'triggers'           => ['type' => 'array', 'items' => ['type' => 'string']],
            'auth_credential_id' => ['type' => 'integer'],
            'custom_headers'     => ['type' => 'array', 'items' => ['type' => 'object', 'properties' => ['key' => ['type' => 'string'], 'value' => ['type' => 'string']]], 'description' => 'List of {key, value} pairs. A value that is a dot-path into the payload is resolved at dispatch; anything else is sent literally.'],
            'url_params'         => ['type' => 'array', 'items' => ['type' => 'object', 'properties' => ['key' => ['type' => 'string'], 'value' => ['type' => 'string']]], 'description' => 'Query-string parameters as a list of {key, value} pairs. A value that is a dot-path into the payload is resolved at dispatch; anything else is sent literally.'],
            ...
            'retry_limit'        => ['type' => 'integer', 'description' => 'Delivery attempts before the job permanently fails, 1-100. OMIT unless the user asks for a specific number — leaving it out inherits the site-wide setting, then the default of 5.'],
            'backoff_strategy'   => ['type' => 'string', 'enum' => ['exponential', 'linear', 'fixed'], 'description' => 'How the wait between retries grows: exponential (base x 2^attempt, capped), linear (base x attempt), fixed (always base). Omit to inherit the site-wide setting, then exponential.'],
            'backoff_base_delay' => ['type' => 'integer', 'description' => 'Seconds the strategy multiplies, 1-86400. Omit to inherit, then 30.'],
            'backoff_max_delay'  => ['type' => 'integer', 'description' => 'Ceiling in seconds for exponential backoff, 1-86400. Omit to inherit, then 3600.'],
          ],
      ...
      'set_conditions' => [
        ...
        'description'  => __('Set conditional-dispatch rules for a webhook+trigger so only matching events leave the site. conditions MUST be an object {"enabled":true,"type":"and"|"or","rules":[{"field":"<dot.path into the captured payload>","operator":"equals|not_equals|contains|not_contains|greater_than|less_than|is_empty|is_not_empty|is_true|is_false|array_contains|object_contains","value":"..."}]}. A rule may add an optional "cast":"number"|"string"|"boolean"|"stringify" to coerce the payload value before comparing (e.g. numeric compare on a string field). A rules item may also be a nested group {"type":"group","match":"and"|"or","rules":[...]}, and there is no limit on how many rules a webhook may carry — use as many as the user\'s logic actually needs. conditions_evaluate_on decides WHICH payload the field paths address, so set it deliberately: "original" (default) = the raw captured payload, so paths look like "args.0.form_id" — read them from get_trigger_schema; "transformed" = the payload AFTER the field mapping AND after any pre-dispatch Code Glue snippet, so paths are the mapping\'s target names plus any key the snippet injects — use this, and ONLY this, when filtering on something a snippet computes. Mixing them up silently breaks the filter: a path that does not exist evaluates to null, and the NEGATIVE operators (not_equals, not_contains, is_empty) then PASS, so the event is dispatched rather than filtered out. For a rule that must fail closed when its field is missing, prefer a positive operator (is_internal equals "false" rather than is_internal not_equals "true").', 'flowsystems-webhook-actions'),
        'category'     => 'webhook-actions',
        'scope'        => AuthHelper::SCOPE_FULL,
        'input_schema' => [
          'type'       => 'object',
      ...
      'list_notification_channels' => [
        'label'        => __('List notification channels', 'flowsystems-webhook-actions'),
        'description'  => __('List the notification channels configured on this site (email, Slack, Discord, Telegram, Teams, SMS, PagerDuty, …) by id, name and type. Rules reference channels by id. Channels hold secrets and are created only in the admin UI (Notifications → Channels) — if none fits, tell the user to add one there rather than inventing an id.', 'flowsystems-webhook-actions'),
        'category'     => 'webhook-actions',
        'scope'        => AuthHelper::SCOPE_READ,
        'input_schema' => ['type' => 'object', 'properties' => (object) []],
        'callback'     => [$reads, 'listNotificationChannels'],
      ],
      'list_notification_rules' => [
        'label'        => __('List notification rules', 'flowsystems-webhook-actions'),
        'description'  => __('List notification rules: site-wide ones (webhook_id null) or a webhook\'s own (pass webhook_id). Each rule = event + filters + channel_ids + optional template.', 'flowsystems-webhook-actions'),
        'category'     => 'webhook-actions',
        'scope'        => AuthHelper::SCOPE_READ,
        'input_schema' => [
          'type'       => 'object',
          'properties' => ['webhook_id' => ['type' => 'integer', 'description' => 'Omit for site-wide rules.']],
        ],
        'callback'     => [$reads, 'listNotificationRules'],
      ],
      'create_notification_rule' => [
        'label'            => __('Create a notification rule', 'flowsystems-webhook-actions'),
        'description'      => __('Alert someone when a delivery reaches a state. event: success | recovered | failed_attempt | retry_scheduled | permanently_failed | skipped. webhook_id scopes the rule to one webhook (use {{step_N.id}} for a webhook created earlier in the plan); omit it for a site-wide rule. channel_ids come from list_notification_channels. filters (all optional): attempts [1,3] (which attempt numbers), http_codes ["5xx","429","transport"], reason exhausted|non_retryable (permanently_failed only), triggers ["woocommerce_*"], only_after_retry, include_tests. template (optional) = {subject, title, body, short} using {{ path | modifier }} placeholders — see the NOTIFICATION TEMPLATES section; unknown paths are rejected with the list of what is available, so read get_trigger_schema first. throttle_seconds quiets repeats per webhook; digest hourly|daily batches them.', 'flowsystems-webhook-actions'),
        'category'         => 'webhook-actions',
        'scope'            => AuthHelper::SCOPE_FULL,
        'requires_confirm' => false,
        'input_schema'     => [
          ...
          'properties' => [
            'webhook_id'       => ['type' => 'integer'],
            'name'             => ['type' => 'string'],
            'event'            => ['type' => 'string', 'enum' => \FlowSystems\WebhookActions\Services\Notifications\DeliveryEvents::ALL],
            'channel_ids'      => ['type' => 'array', 'items' => ['type' => 'integer']],
            'filters'          => ['type' => 'object', 'properties' => [
              'attempts'         => ['type' => 'array', 'items' => ['type' => 'integer']],
              'http_codes'       => ['type' => 'array', 'items' => ['type' => 'string']],
              'reason'           => ['type' => 'string', 'enum' => ['exhausted', 'non_retryable']],
              'triggers'         => ['type' => 'array', 'items' => ['type' => 'string']],
              'only_after_retry' => ['type' => 'boolean'],
            ...
            'digest'           => ['type' => 'string', 'enum' => ['hourly', 'daily']],
            'is_enabled'       => ['type' => 'boolean', 'default' => true],
          ],
          'required'   => ['event', 'channel_ids'],
        ],
        'callback'         => [$writes, 'createNotificationRule'],
      ],
      'update_notification_rule' => [
        'label'            => __('Update a notification rule', 'flowsystems-webhook-actions'),
        'description'      => __('Change a notification rule: its event, channels, filters, template, throttle, digest or enabled state. Same fields as create_notification_rule plus id.', 'flowsystems-webhook-actions'),
        'category'         => 'webhook-actions',
        'scope'            => AuthHelper::SCOPE_FULL,
        'requires_confirm' => false,
        'input_schema'     => [
        ...
        'callback'         => [$writes, 'updateNotificationRule'],
      ],
      'test_notification_rule' => [
        'label'            => __('Send a test notification', 'flowsystems-webhook-actions'),
        'description'      => __('Render a rule against the webhook\'s captured example (or a real delivery log via log_id) and SEND it to every channel of the rule right now. Requires confirmation because a real message goes out.', 'flowsystems-webhook-actions'),
        'category'         => 'webhook-actions',
        'scope'            => AuthHelper::SCOPE_FULL,
        'requires_confirm' => 'always',
        'confirm_notice'   => __('This sends a REAL message to every channel on the rule (email, Slack, SMS, …). Confirm to send it.', 'flowsystems-webhook-actions'),
        'input_schema'     => [
          'type'       => 'object',
          'properties' => [
            'id'        => ['type' => 'integer'],
        ...
        'callback'         => [$tests, 'testNotificationRule'],
      ],
      'assign_credential' => [
