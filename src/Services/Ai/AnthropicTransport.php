...
/**
 * Bring-your-own-key transport for Anthropic's Messages API.
 *
...
class AnthropicTransport implements LlmTransportInterface {
  ...
  public function generateText(string $system, array $messages, array $options = []): string|WP_Error {
    ...
    return $text !== '' ? $text : new WP_Error('fswa_anthropic_empty', __('The AI provider returned no text.', 'flowsystems-webhook-actions'));
  }

  public function id(): string {
    return 'anthropic_byok';
  }
  ...
  /**
   * Coerce transcript turns into the Messages API shape (role + string content).
   *
