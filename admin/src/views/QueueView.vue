...
import { Play, Trash2, RefreshCw, Clock, RotateCcw, ChevronLeft, ChevronRight } from 'lucide-vue-next'
...
const loadQueue = async () => {
  ...
  try {
    const [queueResponse, statsResponse] = await Promise.all([
      api.queue.list({ page: page.value, per_page: perPage.value }),
      api.queue.stats(),
    ])
...
<template>
  <div>
    ...
    <div class="grid grid-cols-3 sm:grid-cols-5 gap-2 sm:gap-4 mb-6">
      ...
      <Card class="p-2 sm:p-4">
        <div class="text-lg sm:text-2xl font-bold text-blue-600">{{ stats.pending }}</div>
        <div class="text-xs sm:text-sm text-muted-foreground">Pending</div>
      </Card>
      <Card class="p-2 sm:p-4">
        <div class="text-lg sm:text-2xl font-bold text-yellow-600">{{ stats.processing }}</div>
        <div class="text-xs sm:text-sm text-muted-foreground">Processing</div>
      </Card>
      <Card class="p-2 sm:p-4">
        <div class="text-lg sm:text-2xl font-bold text-orange-600">{{ stats.due_now }}</div>
        <div class="text-xs sm:text-sm text-muted-foreground">Due Now</div>
      </Card>
      <Card class="p-2 sm:p-4">
        <div class="text-lg sm:text-2xl font-bold text-red-600">{{ stats.failed }}</div>
    ...
    <!-- Loading -->
    <Card v-else-if="loading" class="p-8 text-center">
