...
<template>
  <div class="mb-4 space-y-2">
    ...
    >
      <AlertTriangle class="w-3.5 h-3.5 shrink-0" />
      <span>{{ __('Queue processor has never run. Ensure WP-Cron is active or configure an external cron job for reliable delivery.') }}</span>
    </div>
    ...
    <div v-else class="flex items-center gap-3 sm:gap-6 flex-wrap">
      ...
      >
        ...
        <span class="text-xs sm:text-sm font-medium text-primary"
          >{{ __('Ready to send some data?') }}</span
        >
      </div>
      ...
      <template v-if="currentContext === 'webhooks'">
        <div class="flex items-center gap-1.5 sm:gap-2 text-xs sm:text-sm">
          ...
          <span class="text-muted-foreground">{{ __('Active/Total') }}</span>
        ...
        <div class="flex items-center gap-1.5 sm:gap-2 text-xs sm:text-sm">
          ...
          <span class="text-muted-foreground">{{ __('Total sent') }}</span>
          <span class="font-medium tabular-nums">{{
        ...
        <div class="flex items-center gap-1.5 sm:gap-2 text-xs sm:text-sm">
          ...
          <span class="text-muted-foreground">{{ __('Avg response') }}</span>
          <span class="font-medium tabular-nums">{{
      ...
      <template v-else-if="currentContext === 'logs'">
        <div class="flex items-center gap-1.5 sm:gap-2 text-xs sm:text-sm">
          ...
          <span class="text-muted-foreground">{{ __('Success') }}</span>
        ...
        <div class="flex items-center gap-1.5 sm:gap-2 text-xs sm:text-sm">
          ...
          <span class="text-muted-foreground">{{ __('Errors') }}</span>
        ...
        <div class="flex items-center gap-1.5 sm:gap-2 text-xs sm:text-sm">
          ...
          <span class="text-muted-foreground">{{ __('Total (7d)') }}</span>
      ...
      <template v-else-if="currentContext === 'queue'">
        <div class="flex items-center gap-1.5 sm:gap-2 text-xs sm:text-sm">
          <Zap class="w-3.5 h-3.5 sm:w-4 sm:h-4 text-muted-foreground" />
          <span class="text-muted-foreground">{{ __('Last hour') }}</span>
          <span class="font-medium tabular-nums">{{
        ...
        <div class="flex items-center gap-1.5 sm:gap-2 text-xs sm:text-sm">
          <Activity class="w-3.5 h-3.5 sm:w-4 sm:h-4 text-muted-foreground" />
          <span class="text-muted-foreground">{{ __('Today') }}</span>
          <span class="font-medium tabular-nums">{{ formatNumber(velocity.last_day) }}</span>
        </div>
        <div class="flex items-center gap-1.5 sm:gap-2 text-xs sm:text-sm">
          <Clock class="w-3.5 h-3.5 sm:w-4 sm:h-4 text-yellow-500" />
          <span class="text-muted-foreground">{{ __('Pending') }}</span>
        ...
        <div v-if="observability.avg_attempts_per_event > 0" class="flex items-center gap-1.5 sm:gap-2 text-xs sm:text-sm">
          <Activity class="w-3.5 h-3.5 sm:w-4 sm:h-4 text-muted-foreground" />
          <span class="text-muted-foreground">{{ __('Avg attempts') }}</span>
