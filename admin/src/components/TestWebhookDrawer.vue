...
const send = async (mode) => {
  ...
  try {
    ...
    sendError.value = e.message ?? __('Test failed.')
  } finally {
...
const nowStatus = computed(() => {
  ...
  if (!code)                        return { label: __('Failed'),       icon: XCircle,       bar: 'bg-red-50    dark:bg-red-950    text-red-800    dark:text-red-300    border-b border-red-200    dark:border-red-800'    }
  if (code >= 200 && code < 300)    return { label: __('Success'),      icon: CheckCircle2,  bar: 'bg-green-50  dark:bg-green-950  text-green-800  dark:text-green-300  border-b border-green-200  dark:border-green-800'  }
  if (code >= 300 && code < 400)    return { label: __('Redirect'),     icon: AlertTriangle, bar: 'bg-yellow-50 dark:bg-yellow-950 text-yellow-800 dark:text-yellow-300 border-b border-yellow-200 dark:border-yellow-800' }
  if (code >= 400 && code < 500)    return { label: __('Client Error'), icon: AlertTriangle, bar: 'bg-orange-50 dark:bg-orange-950 text-orange-800 dark:text-orange-300 border-b border-orange-200 dark:border-orange-800' }
  return                                   { label: __('Server Error'), icon: XCircle,       bar: 'bg-red-50    dark:bg-red-950    text-red-800    dark:text-red-300    border-b border-red-200    dark:border-red-800'    }
})
      ...
      <div class="relative z-[100001] flex flex-col w-full max-w-lg bg-background border-l border-border shadow-xl overflow-y-auto">
        ...
        <div class="flex items-center justify-between px-6 py-4 border-b border-border shrink-0">
          <div class="flex items-center gap-2">
            ...
            <span class="font-semibold text-sm">{{ __('Test Webhook') }}</span>
        ...
        <div class="flex-1 px-6 py-5 space-y-5">
          ...
          >
            ...
            <span v-html="sprintf(__('You have unsaved changes. The test will run against the %1$ssaved%2$s version of this webhook.'), '<strong>', '</strong>')"></span>
          </div>
          ...
          <div v-if="!singleTrigger" class="space-y-1.5">
            ...
            <Select v-model="selectedTrigger">
              <SelectTrigger class="w-full">
                <SelectValue :placeholder="__('Select trigger')" />
              </SelectTrigger>
              <SelectContent>
          ...
          <div v-else class="flex items-center flex-wrap gap-2 text-sm text-muted-foreground">
            <span>{{ __('Trigger:') }} <span class="font-mono text-foreground">{{ triggers[0] }}</span></span>
            <Badge v-if="hasPreGlue" variant="default" class="text-xs">
              <Code2 class="h-3 w-3 mr-1" />
              {{ __('Pre Glue') }}
            </Badge>
            ...
            <RadioGroup v-model="source" class="space-y-2">
              <div class="flex items-start gap-2">
                ...
                <label for="src-captured" class="cursor-pointer">
                  <div class="text-sm font-medium">{{ __('Captured') }}</div>
                  <div class="text-xs text-muted-foreground">{{ __('Last captured payload for this trigger') }}</div>
                </label>
              </div>
              <div class="flex items-start gap-2">
                <RadioGroupItem id="src-mapped" value="mapped" class="mt-0.5" />
                <label for="src-mapped" class="cursor-pointer">
                  <div class="text-sm font-medium">{{ __('Captured + Mapping') }}</div>
              ...
              <div class="flex items-start gap-2">
                ...
                <label for="src-custom" class="cursor-pointer">
                  <div class="text-sm font-medium">{{ __('Custom') }}</div>
                  <div class="text-xs text-muted-foreground">{{ __('Write your own JSON payload') }}</div>
                </label>
              </div>
            </RadioGroup>
          </div>
          ...
          <div v-if="source === 'custom'" class="space-y-1.5">
            <Label>{{ __('Custom Payload') }}</Label>
            <JsonEditor v-model="customPayload" />
            <p v-if="!isValidJson" class="text-xs text-destructive">{{ __('Invalid JSON') }}</p>
          </div>
          ...
          <div v-if="result?.mode === 'now'" class="rounded-md border overflow-hidden">
            ...
            <div :class="['flex items-center gap-2 px-3 py-2 text-sm font-medium', nowStatus.bar]">
              ...
              <span v-if="result.log?.http_code" class="font-mono text-xs opacity-70">{{ sprintf(__('HTTP %s'), result.log.http_code) }}</span>
              <span v-if="result.log?.duration_ms" class="text-xs opacity-70 ml-auto">{{ result.log.duration_ms }}ms</span>
            </div>
            <!-- Request details -->
            <div class="p-3 border-b border-border">
              <div class="text-xs text-muted-foreground mb-1.5">{{ __('Request') }}</div>
              ...
              <div v-if="result.log?.request_headers && Object.keys(result.log.request_headers).length" class="mb-2">
                <div class="text-xs text-muted-foreground mb-1">{{ __('Headers') }}</div>
              ...
              <span v-else class="text-xs text-muted-foreground italic">{{ __('No body sent') }}</span>
            </div>
            <!-- Response body -->
            <div v-if="result.log?.response_body" class="p-3 border-b border-border">
              <div class="text-xs text-muted-foreground mb-1">{{ __('Response') }}</div>
            ...
            <!-- Error -->
            <div v-if="result.log?.error_message && nowStatus.label !== __('Success')" class="p-3 border-b border-border">
              <div class="text-xs text-muted-foreground mb-1">{{ __('Error') }}</div>
          ...
          >
            ...
            <div>
              {{ __('Test queued — it will run on the next cron cycle.') }}
              <button class="underline underline-offset-2 ml-1 hover:opacity-80 transition-opacity inline-flex items-center gap-0.5" @click="viewLog">
                {{ __('View in Logs') }} <ExternalLink class="h-3 w-3" />
              </button>
            </div>
          </div>
        ...
        <div class="px-6 py-4 border-t border-border shrink-0 flex flex-wrap gap-2">
          ...
          >
            ...
            {{ sendingNow ? __('Sending…') : __('Run Test Now') }}
          </Button>
          <Button
            variant="outline"
          ...
          >
            <Clock class="h-4 w-4" />
            {{ sendingQueue ? __('Queuing…') : __('Queue Test') }}
          </Button>
          <Button variant="ghost" class="ml-auto" @click="emit('close')">{{ __('Close') }}</Button>
        </div>
      </div>
    </div>
  </Teleport>
